...
# 4. Пароль БД
read -sp "Пароль для базы данных (скроется при вводе, Enter = авто): " DB_PASSWORD
echo ""
...
read -p "GEMINI API Key (опционально, Enter чтобы пропустить): " GEMINI_API_KEY
...
read -p "Ваш домен (например: myshop.com, Enter чтобы пропустить): " DOMAIN
if [ ! -z "$DOMAIN" ]; then
