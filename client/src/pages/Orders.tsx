...
const formatDeliveryDate = (
	...
	const deliveryDate = new Date(
		orderDate.getTime() + estimatedDays * 24 * 60 * 60 * 1000
	)
...
const getDeliveryLabel = (
	order: Order
): { text: string; isBackorder: boolean } => {
	...
	if (order.has_backorder) {
		return {
			text: order.backorder_delivery_date
				? new Date(order.backorder_delivery_date).toLocaleDateString('ru-RU', {
						day: 'numeric',
						month: 'long',
				  })
...
export default function Orders() {
	...
	const getStatusColor = (status: string): string => {
		return (
			DEFAULT_STATUS_CONFIG[status]?.color ||
			'bg-gray-100 text-gray-800 border-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700'
		)
	}
		...
		try {
			...
			if (existing.length === 0) {
				toast({
					...
					description:
						'К сожалению, все товары из этого заказа больше недоступны',
					variant: 'destructive',
				})
				return
			}
					...
					<div className='space-y-3'>
						{filteredOrders.map(order => {
							...
							return (
								<Card
									key={order.id}
									className={`overflow-hidden transition-all duration-500 ease-out border-border/50 ${
								...
								>
									...
									>
										<div className='flex items-start gap-2'>
											<div
												className='relative flex-shrink-0 pt-0.5'
												style={{
													width: order.items.length > 1 ? '52px' : '40px',
													height: '40px',
												}}
											>
												{order.items.length === 1 ? (
													...
													<>
														...
														>
															...
															{/* Бейдж с количеством */}
															<div className='absolute bottom-0.5 right-0.5 min-w-[16px] h-[16px] rounded-full bg-primary text-primary-foreground text-[9px] font-semibold flex items-center justify-center px-1 shadow-sm'>
																{order.items.length}
															</div>
														</div>
													</>
												...
												<div className='flex items-center gap-1.5'>
													<Badge
														...
														className={`${getStatusColor(
															order.status
														)} text-[10px] font-medium px-1.5 py-0 truncate max-w-[120px]`}
													>
																	...
																	{statusSteps.map((step, index) => {
																		...
																		return (
																			...
																			>
																				...
																				<span
																					className={`text-[7px] sm:text-[9px] mt-1.5 text-center leading-tight whitespace-nowrap ${
																						isCompleted
															...
															>
																<div className='flex items-center gap-3'>
																	...
																	<div className='flex-1'>
																		...
																		>
																			{deliveryInfo.isBackorder
																				? 'Ожидаемая дата доставки (под заказ)'
